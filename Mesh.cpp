...
Mesh::Mesh(std::vector <Vertex>& vertices, std::vector <GLuint>& indices, std::vector <Texture>& textures, unsigned int instanceCount, std::vector<glm::mat4> instanceMatrix)
{
...
)
{
	...
	for (unsigned int i = 0; i < textures.size(); i++)
	{
		...
		textures[i].texSlotSetting(shader, (type + num).c_str(), i);
		textures[i].Bind();
	}
